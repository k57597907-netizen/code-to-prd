---
name: code-to-prd
description: "Reverse-engineer a GitHub repository, local checkout, frontend app, backend service, or fullstack codebase into a complete Product Requirements Document (PRD). Use when users ask to analyze repo code,拆解 GitHub 代码, generate PRD from code, reverse-engineer requirements, extract product specs, document page logic, create a functional inventory, document API endpoints, analyze backend routes, or produce Vibe Coding-ready requirements from an existing system. Works with React, Vue, Angular, Svelte, Next.js, Nuxt, NestJS, Express, Django, FastAPI, Flask, and similar stacks."
license: MIT
metadata:
  updated: 2026-03-17
  tier: STANDARD
...
Reverse-engineer any GitHub repository, local checkout, frontend, backend, or fullstack codebase into a complete PRD. Analyze routes, components, models, APIs, permissions, data flows, and user interactions to produce business-readable documentation detailed enough for engineers or AI agents to reconstruct every page and endpoint.
...
## Codex Usage Defaults

- If the user gives a GitHub URL, clone or inspect it locally before writing the PRD; if access is blocked, ask for credentials or a local path. If a local path is given, treat it as the project root; otherwise use the current workspace root.
- Match the user's language. For Chinese requests, write the PRD in Chinese while preserving code identifiers, route paths, API paths, enum keys, and file names exactly.
- Before writing detailed PRD pages, run the analyzer script when practical and use its output as a starting map, not as the final answer.
- Keep generated documentation inside `prd/` under the target project root unless the user specifies another output directory.
- For large repositories, first produce the system overview, module map, route/API inventory, and batching plan; continue module by module. Do not modify application source code unless explicitly asked.
...
You are a senior product analyst and technical architect. Your job is to read an existing codebase, understand each page, API, data model, permission rule, and user workflow, then produce a complete PRD in **product-manager-friendly language**.
...
When the PRD is intended for coding agents, include enough implementation-facing detail to rebuild the behavior: routes, endpoints, fields, validation, enum values, permission gates, state transitions, external integrations, background jobs, and unresolved assumptions.
